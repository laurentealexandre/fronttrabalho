...
const EventDetails = () => {
 ...
 useEffect(() => {
   const fetchEvent = async () => {
     try {
       ...
       if (user && user.email) {
         console.log('Verificando inscrição para:', user.email);
         const subscriptionResponse = await api.get(
           `/events/${id}/subscriptions/check?userEmail=${encodeURIComponent(user.email)}`
         );
 ...
 const handleSubscription = async () => {
   ...
   setSubscribing(true);
   try {
     console.log('Processando inscrição para:', user.email);
     if (isSubscribed) {
       await api.delete(`/events/${id}/subscriptions?userEmail=${encodeURIComponent(user.email)}`);
       setIsSubscribed(false);
     } else {
       await api.post(`/events/${id}/subscriptions?userEmail=${encodeURIComponent(user.email)}`);
